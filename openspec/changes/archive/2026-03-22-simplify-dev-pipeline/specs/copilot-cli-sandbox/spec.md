...
The system SHALL provision a dedicated Azure Container App for running the GitHub Copilot CLI in a Docker sandbox with autopilot mode enabled. The sandbox Container App SHALL be isolated from the backend Container App and expose an HTTP API for receiving task commands. The sandbox SHALL NOT include the openspec CLI.

#### Scenario: Sandbox Container App deployed
- **WHEN** infrastructure is provisioned via `azd up`
- **THEN** a separate Container App named `sandbox` is created with Docker-in-Docker support, pre-installed GitHub Copilot CLI (without openspec), and inbound access restricted to the backend Container App only
...
#### Scenario: Live output during Sequential mode
- **WHEN** a Sequential dev task is executing features sequentially
- **THEN** output from each feature's implement stage SHALL be distinguishable in the stream, labeled by feature name

### Requirement: Copilot CLI invocation format
The sandbox SHALL execute Copilot CLI commands using the `--autopilot --yolo --experimental` flags for autonomous implementation. The `--agent squad` flag routes through squad-pr for team coordination. The `--continue` flag preserves context between sequential invocations.

#### Scenario: Mockup implementation command
- **WHEN** a mockup implement stage executes
- **THEN** the sandbox runs: `copilot --autopilot --yolo --experimental --model <model> --agent squad -p "<prompt>"`

#### Scenario: Sequential feature with context continuation
- **WHEN** a sequential feature implement stage executes after foundation
- **THEN** the sandbox runs: `copilot --autopilot --yolo --experimental --model <model> --agent squad --continue -p "<feature prompt>"`
...
#### Scenario: Download after Sequential completion
- **WHEN** a Sequential dev task completes successfully
...
- **WHEN** a dev task (Mockup or Sequential) finishes building
...
### Requirement: OpenSpec CLI in sandbox
**Reason**: The openspec CLI is no longer needed. The Copilot CLI `--autopilot` flag handles all project scaffolding, implementation, and iteration autonomously.
**Migration**: Remove openspec from sandbox Docker image. Remove `openspec init`, `openspec list --json`, and all openspec skill invocations from dev_agent.py.
