...
#### Scenario: Supervisor routes a research task
- **WHEN** the supervisor receives a request related to research (create_research, get_research_list, get_research, delete_research, web_search, deep_research)
- **THEN** the supervisor SHALL route the request to the research agent
- **AND** return the research agent's response and agent name to the caller
...
- **THEN** the Voice Live session SHALL include function tool definitions from all registered agents (notes, brainstorm, and research)

#### Scenario: Voice model invokes research function
- **WHEN** the voice model determines the user wants to do research
- **THEN** the model SHALL call the appropriate research function tool (web_search, deep_research, etc.)
