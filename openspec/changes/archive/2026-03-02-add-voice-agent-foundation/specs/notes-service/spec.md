...
- **THEN** the document SHALL contain: `id` (UUID), `userId` (partition key), `title` (string, required), `content` (string, required), `docType` ("note"), `createdAt` (ISO 8601), `updatedAt` (ISO 8601)
...
- **WHEN** a create request is received with title and content
...
- **WHEN** an update request is received with a note ID and partial fields
...
- **WHEN** a POST request is made to `/api/notes` with `title` and `content`
- **THEN** the API SHALL create the note and return it with 201 status

#### Scenario: PUT /api/notes/{id}
- **WHEN** a PUT request is made to `/api/notes/{id}` with updated fields
