...
class NotesAgent:
    ...
    async def handle_function_call(self, function_name: str, arguments: str) -> str:
        ...
        if function_name == "create_note":
            note = await self._service.create(
            ...
            notes = await self._service.list()
            return json.dumps(
                {
                    "notes": [
            ...
            note = await self._service.get_by_id(args["note_id"])
            if note:
                return json.dumps(
            ...
            note = await self._service.update(args["note_id"], update)
            if note:
                return json.dumps(
                    {"success": True, "note": {"id": note.id, "title": note.title}},
                )
            ...
            deleted = await self._service.delete(args["note_id"])
