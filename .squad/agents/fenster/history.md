...
- PDF export local fallback: `_export_slides_pdf` in `dev_agent.py` now tries blob storage first, then falls back to `uploads/exports/{task_id}/slides.pdf` on the local filesystem. The `download_pdf` route in `dev.py` detects `/uploads/...` URLs and serves from disk instead of blob. This follows the same dual-path pattern as profile photos (blob → local).
