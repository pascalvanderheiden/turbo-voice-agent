...
## Learnings — 2026-05-27 dev-task mockup pipeline investigation
- Mockup dev-task stages are declared in both dev services as `init → skills → implement → screenshots` (`backend/app/services/dev_service.py`, `backend/app/services/cosmos_dev_service.py`). Sequential mode is `init → skills → implement-foundation → implement-feature-N → screenshots`; slides mode is `init → run → slides`.
- Runtime dispatch lives in `backend/app/agents/dev_agent.py`: `run_pipeline()` chooses sequential only when `mode in ("sequential", "openspec")` and there is more than one iteration, slides for `mode == "slides"`, otherwise mockup.
- Mockup spec flow is: `create_dev_task(spec_id)` stores `specId`, `_populate_iterations_from_spec()` replaces the default iteration label/stages, `_run_mockup_pipeline()` calls `_get_spec_content(task.spec_id, user_id)`, `_extract_mockup_description()` extracts `## Mockup Description` if present, then sends only that text to Copilot CLI.
- The mockup init stage deliberately calls `_run_squad_stage()` before skills/implement. `_run_squad_stage()` literally executes `squad init`, then writes generated `.squad/*` config, hires generated agents, runs `squad doctor`, and stores squad metadata. Therefore `.squad/templates/*`, `.squad/casting/*`, and related Squad files in a mockup checkpoint can be explained by the built-in init behavior, not necessarily by the implement prompt.
- The implement stage itself does not run `squad init`; it posts a Copilot prompt to sandbox `/tasks` with `prompt=mockup_desc`, `agent="squad"`, `autopilot=True`, `workDir=/workspace/{task_id}`. The sandbox converts any prompt into `copilot -p <prompt> --model <model> --autopilot --yolo --experimental [--agent squad]`.
- `_get_spec_content()` silently returns an empty string when `specId` is missing, spec service is unavailable, Cosmos read fails, or spec is not found. There is no guard before `_run_squad_stage()` or implement, so an empty/missing spec still runs `squad init`, then invokes Copilot with no meaningful prompt, and later server startup falls back to `npx serve` over whatever files exist.
- Sandbox `server.js` has no special handling for `mockup-implement`; it only distinguishes raw `command` from Copilot `prompt`. `/tasks` streams stdout/stderr, `/skills/sync` syncs user skills, `/files` exposes artifacts, and `/proxy/:port/*` proxies preview traffic.
- Multi-stage pipelines must fail-fast on stage errors; readiness checks must demand success codes, not just non-server-errors. Loud failures beat misleading success.
