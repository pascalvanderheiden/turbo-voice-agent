...
- **2026-05-27:** Dev-task event pipeline: sandbox `server.js` wraps child process stdout/stderr as SSE entries from `/tasks/{sandboxTaskId}/stream`; backend `_sandbox_exec` consumes that stream, appends entries to the in-memory per-dev-task pipeline buffer, and `routes/dev.py` exposes that buffer as `/api/dev/{taskId}/stream`; the frontend detail page opens an `EventSource` to that backend SSE, renders only stdout/stderr/stage/decision terminal lines, and separately polls `devApi.get()` for stage/squad state. Production logs confirmed backend stage execution for task `d9cc6118-7805-45b0-9433-1c38a5c8af56`, but the UI has no structured agent-event contract: `/api/dev/{id}/stream` emits anonymous `data:` payloads only, the agents page does not subscribe to dev-task streams, and squad activity is best-effort parsed from stdout rather than explicit `agent_start` / `agent_progress` events. Root-cause hypothesis: expected agent-level events are not missing in frontend filtering; they are not produced as structured events end-to-end yet.
