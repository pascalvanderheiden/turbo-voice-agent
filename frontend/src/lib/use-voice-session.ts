...
export function useVoiceSession() {
    ...
    try {
      ...
      ws.onmessage = (event) => {
        ...
        switch (msg.type) {
          ...
          case "error":
            // Only treat as fatal if we're still connecting
            // During active session, log but don't change state
      ...
      ws.onclose = () => {
        // Only set disconnected if we haven't already cleaned up
  ...
  }, [playAudioQueue, updateConnectionState]);
  ...
  const cleanupResources = useCallback(() => {
    ...
    if (wsRef.current) {
      ...
      wsRef.current = null;
      // Remove handlers before closing to avoid race
      ws.onclose = null;
      ws.onerror = null;
      ws.onmessage = null;
      try { ws.close(); } catch {}
