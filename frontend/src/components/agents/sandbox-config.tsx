...
import { IconServer, IconRefresh, IconCheck, IconX, IconLoader2, IconSparkles, IconPlayerStop, IconPlayerPlay } from "@tabler/icons-react";
        ...
        <div className="flex gap-2">
          {status === "ready" || status === "busy" ? (
            <button
              onClick={handleStop}
              disabled={loading}
              className="flex flex-1 items-center justify-center gap-2 rounded-[var(--radius-md)] bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 px-4 py-2 text-sm text-red-400 disabled:opacity-50 transition-colors"
            >
              {loading ? <IconLoader2 size={14} className="animate-spin" /> : <IconPlayerStop size={14} />}
              Stop
            </button>
          ) : (
            <button
              onClick={handleStart}
              disabled={loading}
              className="flex flex-1 items-center justify-center gap-2 rounded-[var(--radius-md)] bg-green-500/10 hover:bg-green-500/20 border border-green-500/20 px-4 py-2 text-sm text-green-400 disabled:opacity-50 transition-colors"
            >
              {loading ? <IconLoader2 size={14} className="animate-spin" /> : <IconPlayerPlay size={14} />}
              Start
            </button>
          )}
          <button
